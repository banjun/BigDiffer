...
final class DeepDiffSingleSectionTableViewController: UITableViewController, UISearchResultsUpdating {
    ...
    private func diffAndPatch(old: [Item], new: [Item]) {
        ...
        let diff = DeepDiff.diff(old: old, new: new)
        let diffed = Date()

        // tableView.reload(changes: diff, section: 0, completion: {_ in})
        tableView.reloadWithOptimizations(changes: diff, section: 0, completion: {_ in})
        let applied = Date()
