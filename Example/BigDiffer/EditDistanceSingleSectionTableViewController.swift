...
final class EditDistanceSingleSectionTableViewController: UITableViewController, UISearchResultsUpdating {
    ...
    func diffAndPatch(old: [String], new: [String]) {
        ...
        let diff = old.diff.compare(to: new)
        let diffed = Date()

        tableView.diff.reloadWithOptimization(with: diff, fallbackBlock: {tableView.reloadData()})
        let applied = Date()
