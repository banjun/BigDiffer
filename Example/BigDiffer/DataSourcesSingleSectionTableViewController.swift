...
private struct Item: Diffable, Equatable {
    var diffIdentifier: String {return value}
    typealias Identifier = String

    let value: String
}

final class DataSourcesSingleSectionTableViewController: UITableViewController, UISearchResultsUpdating {
    ...
    private lazy var sectionDataController: SectionDataController<Item, TableViewAdapter> = SectionDataController(adapter: TableViewAdapter(tableView: self.tableView))
    ...
    private func diffAndPatch(old: [Item], new: [Item]) {
        ...
        sectionDataController.update(items: new, updateMode: .partial(animated: true), immediately: true, completion: {})
