...
extension UITableView {
    ...
    public func apply(
        ...
        deleteSections(.init(bigDiff.deletedSectionIndices), with: deletionAnimation)
        deleteRows(at: bigDiff.deletionsInSections, with: deletionAnimation)
        bigDiff.sectionMoves.forEach {
            ...
            deleteSections([$0.from], with: deletionAnimation)
            insertSections([$0.to], with: insertionAnimation)
            // moveSection($0.from, toSection: $0.to)
        }
        insertSections(.init(bigDiff.insertedSectionIndices), with: insertionAnimation)
        insertRows(at: bigDiff.insertionsInSections, with: insertionAnimation)
        reloadSections(.init(bigDiff.reloadableSectionIndices + bigDiff.fallbackedToReloadSectionIndices), with: .automatic)
    }
